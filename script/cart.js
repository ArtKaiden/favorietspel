...
function UpdateItems() {
  CheckForNullOr0();
  items1.textContent = `${pcAmount}`;
  items2.textContent = `${psAmount}`;
  items3.textContent = `${xboxAmount}`;
  total.textContent = `${Number(pcAmount) + Number(psAmount) + Number(xboxAmount)}`;
